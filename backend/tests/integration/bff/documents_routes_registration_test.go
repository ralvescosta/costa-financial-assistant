...
func TestDocumentsRouteIntegration(t *testing.T) {
	...
	t.Run("list-documents endpoint enforces auth semantics", func(t *testing.T) {
		resp, err := srv.Client().Get(srv.URL + "/api/v1/documents")
		require.NoError(t, err)
		defer resp.Body.Close()
	...
	t.Run("list-documents accepts seeded owner context with omitted pagination", func(t *testing.T) {
		req, err := http.NewRequest(http.MethodGet, srv.URL+"/api/v1/documents", nil)
		require.NoError(t, err)
