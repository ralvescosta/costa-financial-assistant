...
type DocumentsService interface {
	...
	UploadDocument(ctx context.Context, projectID, uploadedBy, fileName string, fileBytes []byte) (*views.DocumentResponse, error)

	// ClassifyDocument updates the kind (bill or statement) of an existing document.
	ClassifyDocument(ctx context.Context, projectID, documentID, kind string) (*views.DocumentResponse, error)

	// ListDocuments returns a project-scoped page of documents.
	ListDocuments(ctx context.Context, projectID string, pageSize int32, pageToken string) (*views.ListDocumentsResponse, error)

	// GetDocument returns full document metadata including extraction fields.
	GetDocument(ctx context.Context, projectID, documentID string) (*views.DocumentDetailResponse, error)
}
...
type ProjectsService interface {
	...
	GetCurrentProject(ctx context.Context, projectID, userID, role string) (*views.ProjectResponse, error)

	// ListMembers returns all members for the caller's project.
	ListMembers(ctx context.Context, projectID, userID, role string, pageSize int32, pageToken string) (*views.ListMembersResponse, error)

	// InviteMember sends an invitation to the given email with the specified role.
	InviteMember(ctx context.Context, projectID, inviterID, inviterRole, email, role string) (*views.ProjectMemberResponse, error)

	// UpdateMemberRole changes the role of an existing project member.
	UpdateMemberRole(ctx context.Context, projectID, callerID, callerRole, memberID, newRole string) (*views.ProjectMemberResponse, error)
}
...
type SettingsService interface {
	...
	ListBankAccounts(ctx context.Context, projectID string) (*views.ListBankAccountsResponse, error)

	// CreateBankAccount registers a new bank account label for the project.
	CreateBankAccount(ctx context.Context, projectID, createdBy, label string) (*views.BankAccountResponse, error)
...
type PaymentsService interface {
	...
	GetPaymentDashboard(ctx context.Context, projectID, userID, cycleStart, cycleEnd string, pageSize int32, pageToken string) (*views.PaymentDashboardResponse, error)

	// MarkBillPaid idempotently marks a bill as paid.
	MarkBillPaid(ctx context.Context, projectID, billID, paidBy string) (*views.MarkBillPaidResponse, error)

	// GetCyclePreference returns the project's preferred payment day.
	GetCyclePreference(ctx context.Context, projectID string) (*views.CyclePreferenceResponse, error)

	// SetCyclePreference creates or updates the project's preferred payment day.
	SetCyclePreference(ctx context.Context, projectID string, dayOfMonth int, updatedBy string) (*views.CyclePreferenceResponse, error)
}
...
type ReconciliationService interface {
	...
	GetSummary(ctx context.Context, projectID, periodStart, periodEnd string) (*views.ReconciliationSummaryResponse, error)

	// CreateManualLink manually links a statement transaction to a bill record.
	CreateManualLink(ctx context.Context, projectID, transactionLineID, billRecordID, linkedBy string) (*views.ReconciliationLinkResponse, error)
}
...
type HistoryService interface {
	...
	GetTimeline(ctx context.Context, projectID string, months int) (*views.TimelineResponse, error)

	// GetCategoryBreakdown returns bill amounts grouped by bill type and month.
	GetCategoryBreakdown(ctx context.Context, projectID string, months int) (*views.CategoryBreakdownResponse, error)

	// GetComplianceMetrics returns on-time vs overdue bill counts and compliance rate.
	GetComplianceMetrics(ctx context.Context, projectID string, months int) (*views.ComplianceResponse, error)
}
